...
def get_next_cell_state(board, x, y):
    if (y,x) in get_corners(board): # Corners are broken so they're always ON
        return 1
...
if __name__ == '__main__':
    ...
    for corner_y, corner_x in get_corners(board): # Adjust input for broken display
        board[corner_y][corner_x] = 1
